...
for gene in db_new.all_features(featuretype="gene"):
    ...
    if new_utrs:
        total_utrs += 1
    if len(new_utrs) > 1:
        peaks2utr_utrs = [f for f in new_utrs if f.source=="peaks2utr"]
        canonical_utrs = [f for f in new_utrs if f.source=="EuPathDB"]
        if any(peaks2utr_utrs) and any(canonical_utrs):
            nUTR = UTR(start=peaks2utr_utrs[0].start, end=peaks2utr_utrs[0].end)
            ...
            if any(peaks2utr_utrs):
                total_new += 1
            elif any(canonical_utrs):
                total_existing +=  1
    elif len(new_utrs) == 1:
        if new_utrs[0].source == "peaks2utr":
            total_new += 1
