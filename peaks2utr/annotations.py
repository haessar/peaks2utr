...
from .utils import cached, iter_batches, feature_from_line
...
class Annotations(collections.UserDict):
    ...
    def __enter__(self):
        self.processes = [self.batch_annotate_strand(batch) for batch in iter_batches(self.peaks, math.ceil(self.total_peaks/self.args.processors))]
    ...
    def _iter_peaks(self, db, peaks_batch):
        for peak in peaks_batch:
            self.annotate_utr_for_peak(db, peak)
    ...
    def batch_annotate_strand(self, peaks_batch):
        ...
        return multiprocessing.Process(target=self._iter_peaks, args=(_db, peaks_batch))

    def annotate_utr_for_peak(self, db, peak):
        """
        ...
        if genes:
            for idx, gene in enumerate(genes):
                ...
                try:                
                    ...
                    criteria.assert_whether_utr_already_annotated(peak, transcript, db, self.args.override_utr, self.args.extend_utr)
