...
from .constants import FeatureTypes, STRAND_CIGAR_SOFT_CLIP_REGEX, GFFUTILS_GFF_DIALECT, GFFUTILS_GTF_DIALECT
...
class UTR(RangeMixin):
    ...
    def _create_id(self, transcript, db):
        existing_utrs = list(db.children(transcript, featuretype=FeatureTypes.ThreePrimeUTR)) + list(db.children(transcript, featuretype=FeatureTypes.FivePrimeUTR))
        if existing_utrs:
